...
GatewayConnector's desktop release targets are macOS and Windows. The current
unsigned manual artifacts are macOS arm64 and Windows x64. Linux desktop is
deferred, unsupported, and non-gating; this repository publishes no Linux
artifact or feed. Portable core/backend source is intentionally retained.
"Browser PKCE" means that the native desktop app opens the operating-system
browser for authentication; GatewayConnector has no browser or WebAssembly
application target.
...
Rust 1.97 is pinned. In an Amp orb, `.agents/setup` installs Rust and Linux
libraries as best-effort developer tooling. Orb checks are useful during
iteration but are not a supported Linux desktop gate or release evidence; the
authoritative full non-visual gate runs on native macOS.

The desktop dependency provenance is pinned exactly: `fran0220/gpui-kit` at
`7689b765aad27261a9f119f8ae6f6ff560f0c79b`, with direct `gpui` and
...
CI has only native macOS and Windows release gates. macOS is the authoritative
full non-visual gate: formatting, all workspace tests (including native core,
backend, isolated-layout, and crash-recovery behavior), all-target/all-feature
clippy with warnings denied, GPUI compilation, and the macOS package contract.
Windows retains native core/app/backend tests, all-feature clippy, GPUI
compilation, release staging, and mutation-sensitive PE/resource/ZIP checks.
Linux is not a CI or release gate.

Build and inspect the unsigned manual macOS arm64 artifact on native Apple
Silicon with:

```bash
./packaging/macos/stage-release.sh
./packaging/macos/assert-release.sh
```

The output is
`dist/GatewayConnector-<version>-macos-arm64.app.tar.gz`, containing
`GatewayConnector.app`, `LICENSE`, and `release-metadata.json`. The bundle has
the metadata-defined identifier/version, a native arm64 executable, and an
`.icns` generated by native macOS tools from the tracked neutral icon source.

Build and inspect the unsigned manual Windows x64 artifact on Windows with:
...
Its output is `dist/GatewayConnector-<version>-windows-x64.zip`, containing
only `gateway-connector.exe`, `LICENSE`, and `release-metadata.json`. It is not
signed. The macOS artifact is also unsigned and not notarized. Neither target
has an updater or latest-release feed. `release-metadata.json` is the exact
neutral packaging source of truth and is checked against the Cargo workspace
version, target architecture, bundle/resource metadata, and archive contents.
The original icon source and its license notice live in `packaging/windows/`;
both target packagers use that neutral source without a network download.

Automated package checks are non-visual. Native Windows and macOS visual
acceptance remain required before calling the desktop client complete. The
